...
from .models import ChatRoom, ChatMessage, UserOnlineStatus, Messaging
...
@receiver(sender=Messaging, signal=post_save)
def send_msg_notification_to_users(sender, instance, created, **kwargs):
    if created:
        # rooms = ChatRoom.objects.filter(Q(user1=to_user) | Q(user2=to_user))
        for user in [instance.receiver, instance.sender]:
            chat_room = f"room_{user.id}"
            data = json.dumps({
                "command": "incoming_message"
            })
            print(f"Sending to {chat_room} {data}")
