...
from administration.models import Recruiter, MentorFeedback, CustomUser, Parent, MentorBookingLeads, DemandAndSupply, \
    TechSupport, ContactUs, Mentor, Institute, FutureStudent, UserCSVImportedData
...
class MentorFilter(django_filters.FilterSet):
    ...
    #                                                     )
    are_you_graduated = django_filters.ChoiceFilter(choices=QUESTION_CHOICE, empty_label='Select Alumni',
                                                    )
    studying_in_countries = django_filters.MultipleChoiceFilter(
        choices=countries, method='filter_countries', label="Alumni currently studying in")
    ...
    UNIVERSITY_START_YEAR_CHOICES = []
    for r in range((datetime.datetime.now().year - 6), (datetime.datetime.now().year + 1)):
        UNIVERSITY_START_YEAR_CHOICES.append((r, r))
    university_start_year = django_filters.ChoiceFilter(choices=UNIVERSITY_START_YEAR_CHOICES,
    ...
    class Meta:
        ...
        fields = ['programme_level', 'currently_studying', 'admin__country', 'studying_in', 'admin__spoken_languages',
                  'admin__first_name','admin__last_name','are_you_graduated', 'mentor_profile_status', 'university_start_year'
                  ]

    def __init__(self, *args, **kwargs):
        super(MentorFilter, self).__init__(*args, **kwargs)
        self.filters['admin__first_name'].label = "First Name"
    ...
    def filter_by_mentor_profile_status(self, queryset, name, value):
        if value == 'Incomplete':
            ...
            apps = Mentor.objects.filter(profile_status=False).exclude(admin=None)
            temp = []
            for i in apps:
                temp.append(i.admin.slug)
            return queryset.filter(admin__user_slug__in=temp, consent4=True)
        elif value == 'Approved':
            apps = Mentor.objects.filter(profile_status=True).exclude(admin=None)
            temp = []
            for i in apps:
                temp.append(i.admin.slug)
            return queryset.filter(admin__user_slug__in=temp)
...
class MentorPersonalFilter(django_filters.FilterSet):
    # available_from = django_filters.DateFilter(widget=DateInput(), method='filter_by_available_date')
    area_of_study_choice = (
        ('', 'Select'),
        (1, 'Business Studies'),
    ...
    class Meta:
        model = MentorPersonalInformation
        fields = ['programme_level', 'admin__country', 'admin__spoken_languages',
                   'name','email','university_start_year',
                  ]
...
class StudentFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        fields = ['name', 'email','admin__country']

    def __init__(self, *args, **kwargs):
        super(StudentFilter, self).__init__(*args, **kwargs)
        self.filters['admin__country'].label = "Country"
...
class StudentSearchFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        fields = ['keyword', 'nationality', 'currently_living_in', 'native_languages']
...
class FutureStudentFilter(django_filters.FilterSet):
    keyword = django_filters.CharFilter(method='filter_by_name', label="Search by name")
    email = django_filters.CharFilter(method='filter_by_email', label="Search by email")
    class Meta:
        model = FutureStudent
...
class UserCSVImportedDataFilter(django_filters.FilterSet):
    class Meta:
