...
from administration.models import CustomUser, Lead, Institute, Student, Mentor, AppAdmin, Recruiter, Parent, School, \
                                    FutureStudent
...
def create_user_profile(sender, instance, created, **kwargs):
    if created:
        ...
        if instance.user_type == 3:
            ...
            student = Student.objects.create(admin=instance, school=SCHOOL_OBJECT)
            ...
            app = Application.objects.create(admin=instance)
            language = EnglishLanguage.objects.create(app=app)
            visa = VisaHistory.objects.create(app=app)
            feedback = ApplicationFeedback.objects.create(app=app)
            reference = Reference.objects.create(app=app)
            experience = ProfessionalExperience.objects.create(app=app)
            training = ProfessionalTrainingCertificate.objects.create(app=app)
            qualification = AcademicQualification.objects.create(app=app)
            statement = PersonalStatement.objects.create(app=app)
            ...
            address = AppAddress.objects.using('afm_personal_information').create(app=app_pi)
            passport = AppPassportInformation.objects.using('afm_personal_information').create(app=app_pi)
            
            print("------------------List of Student Personal Tables created-----------------")
            print("CustomUser: ", sender)
            print("Student: ", student)
            print("Application: ", app)
            print("Application Status: ", app.status)
            print("User(Personal DB): ", user)
            print("English Language:",language)
            print("visa: ", visa)
            ...
            print("Basic Info(Personal DB): ", app_pi)
            print("Address(Personal DB): ",address)
            print("Passport(personal DB): ", passport)      
            print("------------------List of Student Personal Tables created-----------------")
        ...
        if instance.user_type == 4:
            Mentor.objects.create(admin=instance, school=SCHOOL_OBJECT)
            user = CustomUserPersonalInformation.objects.using('afm_personal_information').create(
                user_slug=instance.slug,
                first_name=instance.first_name,
                last_name=instance.last_name)
        ...
        if instance.user_type == 12:
            future_student = FutureStudent.objects.create(admin=instance, school=SCHOOL_OBJECT)
            print("Registration Signal creates Future Student")
            print("Future Student - ", future_student, future_student.id, future_student.school)
...
def on_mentor_update_profile(sender, instance: MentorPersonalInformation, **kwargs):
    # Get Request attribute
    if instance.id is not None:
        print("--------------------------Mentor Profile Completion Email-----------------------------")
        print("Mentor Profile Email signal triggered....")
        ...
        if previous.consent4 != instance.consent4:  # field will be updated
            request = get_current_request()
            mail_send_from = MAIL_SEND_FROM
            website_domain = get_current_site(request).domain
            print("Website Domain - ", website_domain)
