...
function check_if_at_least_one_filed_is_filled(){
    var social_field_values = $('#social_media_section :input').filter(function() {
        return this.value !== "" && this.value !== "0";
    });
    if (social_field_values.length < 1) {
        $('#social_media_section :input')[0].setCustomValidity("Please share at lease one social media profile.");
    }
    else{
        $('#social_media_section :input')[0].setCustomValidity("");
    }
}
if( $('#social_media_section').length ){
    check_if_at_least_one_filed_is_filled();
