...
def mark_all_as_read(request):
    ...
    return redirect('notifications:all_notification')
...
def live_unread_notification_list(request):
    ...
    for notification in request.user.notifications.unread()[0:num_to_fetch]:
        ...
        struct['new_time'] = str(notification)
...
def change_app_notification_url(request):
    notifications = Notification.objects.filter(description__contains='application')
    print(notifications.count())
    for notification in notifications:
        # print(notification.description, notification.target_object_id, notification.target_content_type_id)
        x = notification.description.split('/')
        if Application.objects.filter(id=int(x[-1])).exists():
            app = Application.objects.get(id=int(x[-1]))
            new_url = '/application/application/'+ app.slug
            notification.description = new_url
            notification.save()
            messages.success(request, "Successfully updated url!")
    return redirect('/')