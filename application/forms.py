...
class ApplicationForm(forms.ModelForm):
    about_me = forms.CharField(widget=forms.Textarea(attrs={'rows': 5,
                                                            'placeholder': 'Please write a few lines about your '
                                                                           'interests, hobbies, achievements and what '
                                                                           'sort of courses you are interested in '
                                                                           'studying. (Please do not include any '
                                                                           'personal or contact information here)'}),
                               label="About Applicant"
                               )

    class Meta:
        model = Application
        fields = ['subject', 'program_level', 'intake_month', 'intake_year', 'desire_course', 'fees_affordability',
                  'scholarship_option', 'exclude_institutes', 'exclude_institutes_other', 'study_destination_country',
                  'about_me', 'level_of_english']
...
class AppEducationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            'course_duration_from_month': 'Course Duration From (select month)',
            'course_duration_to_month': 'Course Duration To (select month)',
            'course_duration_from_year': 'Course Duration From (select year)',
            'course_duration_to_year': 'Course Duration To (select year)',
