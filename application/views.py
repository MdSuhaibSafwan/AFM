...
from administration.models import Institute, BlockUser
...
from application.forms import ApplicationForm, AppBasicInformationForm, AppAddressForm, \
    ...
    ApplicationFeedbackForm, NextOfKinInformationForm
...
def application_consent_twfl(request, app_slug):
    ...
    app_form = ApplicationForm(request.POST or None, instance=app, initial=context)
    ...
    return render(request, "application/application_consent.html",
                  {'form': app_form, 'app': app, 'formset': formset_direct_application})
...
def application_english_language_twfl(request, app_slug):
    ...
    if request.method != "POST":
        ...
        return render(request, 'application/application_english_language.html', {'form': form, 'app': app})
    else:
        app_form = AppEnglishLanguageForm(request.POST, request.FILES, instance=english_lan_app)
        if app_form.is_valid():
            app_form.save()
            ...
            print(app_form.errors)
            messages.error(request, "Failed to save, Form is not valid")
            return render(request, "application/application_english_language.html", {'form': app_form, 'app': app})
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            for instance in instances:
                instance.app = app
                instance.save()
            return redirect('application:application_personal_statement_twfl', app.slug)
        else:
            print(formset.errors)
    else:
...
def review_submit_twfl(request, app_slug):
    ...
    if request.method != "POST":
        ...
        clear = app.program_level is not None \
                and basic_info.marital_status is not None \
                and educations[0].institute_name is not None \
                and reference[0].referee_name is not None \
                and common_personal_statement[0].question_1 is not None \
                and visa[0].have_you_studied_in_the_uk_before is not None \
                and app.learning_disabilities_or_difficulties is not None
        ...
        mentor_form = SelectMentorForm(request.POST, instance=app)
        feedback_form = ApplicationFeedbackForm(request.POST, instance=feedback)
        if request.POST.get('mentor'):
            if feedback_form.is_valid() and mentor_form.is_valid():
                feedback_form.save()
                mentor_form.save()
                # Application Status:
                # (0, 'Approved'),
                ...
                verb = 'You have been given a feedback by ' + app.admin.first_name + '(' + app.admin.slug + ')' + \
                       ' for Application submission on Apply for Medicine'
                ...
                # Delete all the junk applications
                delete_null_application_instance_twfl(request)
                return redirect("application:application_submitted_twfl", app.slug)
            else:
            ...
            notify.send(app.admin,
                        ...
                        verb=verb)
            url_name = '#'
            verb = 'Your are selected for application of student %s' % app.admin.first_name
            # notify.send(app.mentor.admin,
            #             recipient=app.mentor.admin,
            #             description=url_name,
            #             target=app,
            #             level='success',verb=verb)
            # Delete all the junk applications
            delete_null_application_instance_twfl(request)
            return redirect("application:application_submitted_twfl", app.slug)
